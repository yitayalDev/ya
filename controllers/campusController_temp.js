...
const transferStaff = async (req, res) => {
  try {
    const { staffId, fromCampusId, toCampusId, toDepartmentId, reason } = req.body;
    ...
    // Verify department belongs to target campus
    const toDepartment = await Department.findById(toDepartmentId).populate('college');
    if (!toDepartment || toDepartment.college.campus.toString() !== toCampusId) {
      return res.status(400).json({ message: 'Invalid department for target campus' });
    }
...
const getStaffByCampus = async (req, res) => {
  try {
    ...
    const staff = await Staff.find({ department: { $in: departmentIds } })
